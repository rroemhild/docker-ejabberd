...
- **SSLCERT_HOST**: SSL Certificate for the hostname.
- **SSLCERT_EXAMPLE_COM**: SSL Certificates for XMPP domains.
...
- **ERL_OPTIONS**: Overwrite additional options passed to erlang while starting ejabberd.

## Modules

- **SKIP_MODULES_UPDATE**: If you do not need to update ejabberd modules specs, skip the update task and speedup start. Defaults to `false`.
...
Use the **LOGLEVEL** environment variable to set verbosity. Defaults to `4` (Info).
